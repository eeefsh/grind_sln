...
{
	// 开始
	entrance << choose_G5X(tc_G5X_in);
	entrance << G90();
	nc_block probe_R(new_BL_Block());
	entrance << (probe_R = 1.50);
	nc_block f_fast(new_BL_Block());
	entrance << (f_fast = 4000.0);
	nc_block f_middle(new_BL_Block());
	entrance << (f_middle = 300.0);
	nc_block f_slow(new_BL_Block());
	entrance << (f_slow = 60.0);

	//旋转A/C轴
	entrance << G01_A_F(tc_A_in, f_fast);
	entrance << G01_C_F(tc_C_in, f_fast);

	//判断是否先到局部安全位置
	nc_block need_back_safe_local(Process_K(0));
	need_back_safe_local << (tc_back_safe_local_in > 0.9);
	...
	entrance << IF_K(need_back_safe_local, back_safe_local_first);

	//仅精探分支
	nc_block no_need_cu(Process_K(0));
	no_need_cu << (abs(tc_need_cu_in) < 0.1);
	...
	only_jing << WAIT();
	only_jing << G01_X_Y_Z_F(ans_xyz_jing_2(0), ans_xyz_jing_2(1), ans_xyz_jing_2(2), f_fast);
	entrance << IF_K(no_need_cu, only_jing);

	//完整探测分支
	//计算粗探开始点并到达
	nc_block need_cu(Process_K(0));
	need_cu << (abs(tc_need_cu_in) > 0.9);
	...
	entrance << IF_K(need_cu, complete_tance);
	...
	entrance << IF_K(need_back_safe_local, back_safe_local_last);
	...
	entrance << Angle_from_a_to_b_around_c(tc_targ_N_in, JC_X,  JC_Y, theta);
	nc_block is_neg_180_degree(Process_K(0));
	is_neg_180_degree << (theta < -179.9);
	entrance << IF_K(is_neg_180_degree, theta = 180.0);
	nc_block error_probe(new_BL_Block());

	double bl_100000 = 100000;
	nc_block bl_100000_block(new_BL_Block());
	entrance << (bl_100000_block = 100000.0);
	nc_block under112d(Process_K(0));
	{
		under112d << (theta < 112.0);
	}
	nc_block nihe_under_112d(Process_K(0));
	{
		nihe_under_112d << (error_probe = ((0.000000005863268 * bl_100000 / bl_100000_block) * theta * theta * theta * theta)
			- ((0.000001238162125 * bl_100000 / bl_100000_block) * theta * theta * theta)
			+ ((0.000099790569297 * bl_100000 / bl_100000_block) * theta * theta)
			- ((0.003495887913656 * bl_100000 / bl_100000_block) * theta)
			+ (0.271230442950323 * bl_100000 / bl_100000_block));
	}
	entrance << IF_K(under112d, nihe_under_112d);

	nc_block over112d(Process_K(0));
	{
		over112d << (theta >= 112.0);
	}
	nc_block nihe_over_112d(Process_K(0));
	{
		nihe_over_112d << (error_probe = ((0.000000008021922 * bl_100000 / bl_100000_block) * theta * theta * theta * theta)
			- ((0.000005342213233 * bl_100000 / bl_100000_block) * theta * theta * theta)
			+ ((0.001350744537916 * bl_100000 / bl_100000_block) * theta * theta)
			- ((0.155220411636639 * bl_100000 / bl_100000_block) * theta)
			+ (7.001242971119470 * bl_100000 / bl_100000_block));
	}
	entrance << IF_K(over112d, nihe_over_112d);

	//输出结果
	entrance << (tc_ans_point_out(0) = tc_inner_X_coorAbs() + error_probe * tc_targ_N_in(0) - tc_targ_N_in(0) * probe_R);
	entrance << (tc_ans_point_out(1) = tc_inner_Y_coorAbs() + error_probe * tc_targ_N_in(1) - tc_targ_N_in(1) * probe_R);
	entrance << (tc_ans_point_out(2) = tc_inner_Z_coorAbs() + error_probe * tc_targ_N_in(2) - tc_targ_N_in(2) * probe_R);
	...
	entrance << IF_K(tc_back_safe_global_in > 0.99, back_anquan);
...
nc_block F_zi_tance_3_axis::choose_G5X(nc_block& G5X)
{
	...
	nc_block is_G54(Process_K(0));
	is_G54 << (G5X < 0.1);
	entr << IF_K(is_G54, G54());
	nc_block is_G55(Process_K(0));
	is_G55 << ((G5X >= 0.1) && (G5X < 1.1));
	entr << IF_K(is_G55, G55());
	nc_block is_G56(Process_K(0));
	is_G56 << ((G5X >= 1.1) && (G5X < 2.1));
	entr << IF_K(is_G56, G56());
	nc_block is_G57(Process_K(0));
	is_G57 << ((G5X >= 2.1) && (G5X < 3.1));
	entr << IF_K(is_G57, G57());
	nc_block is_G58(Process_K(0));
	is_G58 << ((G5X >= 3.1) && (G5X < 4.1));
	entr << IF_K(is_G58, G58());
	nc_block is_G59(Process_K(0));
	is_G59 << ((G5X >= 4.1) && (G5X < 5.1));
