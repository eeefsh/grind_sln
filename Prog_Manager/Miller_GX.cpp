...
void Miller_GX_Base::set_self_dao(Tool_Obj* in_ptr)
{
	ptr_tool = in_ptr;
}
...
void Miller_GX_Base::set_Safe_Point(Point_by_5axis& in_p)
{
	anquandian = in_p;
}
...
void Miller_QDM::set_base_data(CanShu_XinHouMian& in_data)
{
...
inline void Miller_QDM::cal_base_DL()
{
	//1-对前刀面变换--抽样、范围、变化：径向，轴向，法向
	cal_new_mian_data();
	...
	//3-进退刀
	XiDao_QDM_JinTuiDao(DL_base, Mo_Canshu.jintuidao);
	DL_end = DL_base;

	//4-后处理
	daijie_houchuli_a_gx_DL();
...
void Miller_QDM::cal_new_mian_data()
{
	//范围
	double ls_start = Mo_Canshu.range.start;
	double ls_end = Mo_Canshu.range.end;
	Cal_QDM ls_data = ptr_mian->Range(ls_start, ls_end);

	//抽样
	double ls_len = Mo_Canshu.sample.Sample_lenth;
	double ls_ang = Mo_Canshu.sample.Sample_angle;
...
void Miller_QDM::cal_part_DL()
{
	...
	Gpoint Gp;
	int count_p = ptr_mian->n_P();
	for (int i = 0; i < count_p; i++)
	{
		vec ls_p = ptr_mian->BG_P(i);
		vec ls_t = ptr_mian->BG_X(i);
		vec ls_n = ptr_mian->BG_Z(i);
	...
	Bezier_Curve3_QM ls_xhm;
	obj_xinhou.get_data(ls_xhm);

	QuMian_Grovving Grovving;
	Grovving.Set_XHM(&ls_xhm);
	Grovving.Set_QDM(all_Gp);
	Grovving.Set_SL_data(shalun_canshu.R, shalun_canshu.D / 2.0,
		Mo_Canshu.qdm_fangfa.ang_qiexue, !shalun_canshu.in_or_out);//函数默认外侧磨
	...
	for (size_t i = 0; i < ans.size(); i++)
	{
		...
		DL_P ans_DL_P;
		ans_DL_P.G_P = ptr_mian->BG_P(i);
		ans_DL_P.G_SYS_HDM = ptr_mian->BG_SYS(i);
		ans_DL_P.G_SYS_SL = G_SYS_SL;
	...
	//单刃进退刀 
	XiDao_QDM_JinTuiDao(ans_DL_part, Mo_Canshu.jintuidao);

	//刀路点反向，只改点的顺序
	//reverse(DL_part.Data.begin(), DL_part.Data.end());
}

Miller_QDM::Miller_QDM()
{
}
...
Miller_QDM::Miller_QDM(int in_name)
{
	name = in_name;
}

void Miller_HDM::cal_base_DL()
{
	//1-对前刀面变换--抽样、范围、
	cal_new_mian_data();
...
void Miller_HDM::cal_part_DL()
{
	...
	{
		vec ls_p = tcX(3);
		mat ls_sys(3, 3, fill::zeros);
		//选择磨的面
		if (sel_index == 1)
		{
			ls_p = ptr_mian->HJ1_P(i);
			ls_sys = ptr_mian->HJ1_SYS(i);
		}
		else if(sel_index == 1)
		{
			ls_p = ptr_mian->HJ2_P(i);
			ls_sys = ptr_mian->HJ2_SYS(i);
		}
		else if (sel_index == 2)
		{
			ls_p = ptr_mian->HJ3_P(i);
			ls_sys = ptr_mian->HJ3_SYS(i);
		}

		//计算砂轮位姿
		mat G_SYS_SL(3,3,fill::zeros);
		int type_mo = Mo_Canshu.hdm_fangfa.type;
		//旋转+位移
		if (type_mo == 0)
		{
		...
		else if (type_mo == 2)
		{
			...
			double cur_wy = 0.0;
			//确定位移
			if (Mo_Canshu.hdm_fangfa.qx_wy.WY_isbian)
			{
				double cur_len = ptr_mian->Lenth_At_Point(i);
				cur_wy = Mo_Canshu.hdm_fangfa.qx_wy.WY_bian.get(cur_len);
			}
			else
			{
				cur_wy = Mo_Canshu.hdm_fangfa.qx_wy.WY;
			}
			//确定切削
			if (Mo_Canshu.hdm_fangfa.qx_wy.QX_isbian)
			{
				double cur_len = ptr_mian->Lenth_At_Point(i);
				cur_qie = Mo_Canshu.hdm_fangfa.qx_wy.QX_bian.get(cur_len);
			}
			else
			{
				cur_qie = Mo_Canshu.hdm_fangfa.qx_wy.QX;
			}
